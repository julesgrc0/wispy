...
DWORD WINAPI bridge_thread(LPVOID arg)
{
	...
	if (!tmp)
	{
		tmp = new_world(bridge->state->config->max_chunks / 2, bridge->state->config->max_chunks, seed, map_name);
	}
	...
	player->position.x = (world->current->blocks[0].x * cfg->block_size) + (world->current->position * CHUNK_WIDTH * cfg->block_size);
	player->position.y = (world->current->blocks[0].y * cfg->block_size) - (cfg->block_size * 2);
	...
	while (bridge->active)
	{
		...
		{
			bridge->view_current.min.x = floor((camera->target.x - world->current->position * CHUNK_WIDTH * cfg->block_size)/cfg->block_size);
			bridge->view_current.max.x = min(bridge->view_current.min.x + cfg->max_render_block, CHUNK_WIDTH);

			bridge->view_current.max.y = ((int)(camera->target.y / cfg->block_size) % CHUNK_HEIGHT) + cfg->max_render_block;
			
			if (world->next != NULL)
			{
				bridge->view_next.max.x = (int)(bridge->view_current.min.x + cfg->max_render_block)%CHUNK_WIDTH;
		...
		if (bridge->world->next != NULL && bridge->player->position.x > ((position + 1) * CHUNK_WIDTH * cfg->block_size))
		{
		...
		if (abs(player->velocity.x) > min_velocity || player->onground == 0)
		{
			...
			for (unsigned int i = 0; i < CHUNK_LEN; i++)
			{
				...
				if (abs(block_rect.y - player_rect_next.y) > max_blocks_distance || abs(block_rect.x - player_rect_next.x) > max_blocks_distance)
					continue;
		...
		smooth_camera(camera->target.x, player->position.x - (cfg->render_size / 2), base_velocity);
		smooth_camera(camera->target.y, player->position.y - (cfg->render_size / 2), base_velocity);
	}
